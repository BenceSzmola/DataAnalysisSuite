...
%             setXlims(guiobj)
        ...
        function eventDetPlotFcn(guiobj,dTyp,forSpectro,clrAx)
            ...
            elseif ~forSpectro
                ...
                if ~isempty(detBorders)
                    xline(ax,taxis(detBorders(1)),'--g','LineWidth',.75);
                    xline(ax,taxis(detBorders(2)),'--g','LineWidth',.75);
                    hL = dataWin;
                    temp1 = find(tWin==taxis(detBorders(1)));
                    temp2 = find(tWin==taxis(detBorders(2)));
                    hL(1:temp1-1) = nan;
                    hL(temp2+1:end) = nan;
                    plot(ax,tWin,hL,'-r','LineWidth',.75)
                else
                    xline(ax,tStamp,'Color','g','LineWidth',.75);
                end
                ...
                case 'Artifact Suppression'
                    ...
                    switch artSuppName
                        ...
                        case 'Narrow band'
                            ...
                            txt4name = sprintf('NarrowBand(Qf=%d)',qFact);
...
%                         inds2use_interval = discardIntervals4Dets(guiobj,1,data,chan,refch);
            ...
            switch dettype
                case 'GMM+AutocorrFit'
                    ...
                    detinfo.DetSettings.MinLen = algSett.envThrCrossMinLen / fs;
                    detinfo.DetSettings.SdMult = algSett.envThrSdMult;
    ...
    methods (Access = private)
        ...
        function createComponents(guiobj)
            ...
            guiobj.ephysDetPopMenu = uicontrol(guiobj.eventDetTab,...
                ...
                'String',{'--Ephys detection methods--','GMM+AutocorrFit',...
                'CWT based','Adaptive threshold','DoG+InstPow'},...
