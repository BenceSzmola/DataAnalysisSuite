function [data srate] = read_Intan_RHD2000_file(filename)
...
%     temp_sensor_data = temp_sensor_data / 100; % units = deg C
    ...
    srate = frequency_parameters.amplifier_sample_rate; 
    auxrate = frequency_parameters.aux_input_sample_rate; 
    auxrate_ratio = srate/auxrate;
    supplyrate = frequency_parameters.supply_voltage_sample_rate;
    supplyrate_ratio = round(srate/supplyrate);
    
    %%% Board_dig_in hozzáfûzése
    amp_data = cat(1,amp_data,board_dig_in_data);
    
    data.ampdata = amp_data;
...
%     end
    ...
    if (notch_filter_frequency > 0)
        ...
        %%% Notch-filterelt adatokat bele outputba
        data.ampdata(1:5,:) = amplifier_data;
    end
