...
%ha = waitbar(0, ['Reading file... ' str]);

out=gorobj('empty',length(amplifier_channels)+1)';
xname='T';
% I=0; %kimeneti gor számlálója
I = 1;
%measurement date % mehet majd cikluson kivulre
rhd_data.FileStartDate = ['20',filename(end-16:end-11)];  % mert 20xx-es kezdetûevekben jarunk..
rhd_data.FileStartTime = 1000*((str2double(filename(end-9:end-8))*60 + str2double(filename(end-7:end-6))) *60 + str2double(filename(end-5:end-4)));
rhd_data.nsweeps = [];

for j=1:length(amplifier_channels)
%     dt = t_amplifier(2)- t_amplifier(1);
    chi = amplifier_channels(j);

    % rhd_data.chdata = rhd_data.amplifier_data(j,:);
    % rhd_data.timedata = rhd_data.t_amplifier(j,:);
    
%% Gap-free-vel most nem foglalkozunk:
%Gap-free esetén össze kell csatolni a darabokat...
%     if I>1
%         if strcmp(info.operationmode, 'Gap-free')
%             if abs(endtime+dt-chi.starttime)<10^-9
%                 y=get(out(I-1,j), 'extracty');
%                 if length(y)<1500000
%                     %adding to the end of the prev. curve...
%                     out(I-1,j)=set(out(I-1,j), 'y', [y; double(rhd_data.amplifier_data(:))]);
%                     %if ishandle(ha), waitbar(i/info.nsweeps, ha), else ha = waitbar(0,'Reading file...'); end
%                     %endtime=chi.starttime+ rhd_data.t_amplifier(end);
%                     continue
%                 end
%             end
%         end
%     end
    ...
    if rhd_data.FileStartDate==0
        chi.starttime=0;
    else
        date=num2str(rhd_data.FileStartDate);
        time=rhd_data.FileStartTime;
        Y=str2double(date(1:4));
        M=str2double(date(5:6));
        D=str2double(date(7:8));
        chi.starttime =datenum(Y,M,D)+time/24/3600/1000;
    end

    %endtime=chi.starttime+t_amplifier(end);

    name=[filename, '.rdf/'  num2str(j) 'channel'];
%     if j==1, I=I+1; end % ezt majd egyszerusithetjuk, ha itt nincs gap-free szitu
    temp=[];
    temp.name=name;
    temp.csoport=j;
    temp.xunit='ms';
    temp.xname=xname;
    temp.yunit=[char(0181),'V']; % mu kodja
    temp.yname=chi.port_name;
    temp.history='inRHD; ';
    % temp.vars(1)=i;
    % temp.varnames{1}='sweep';
    % if ~iswave
    %    temp.axis=2;
    % else
    %    temp.name=[temp.name '/w'];
    % end
    switch temp.xunit
        case 'ms'
            tfactor = 1000;
    ...
    temp=gorobj('eqsamp', [tfactor*t_amplifier(1), tfactor*(t_amplifier(2)-t_amplifier(1)) ], 'double', amplifier_data(j,:), temp);
    out(I,j)=temp;

    vars=get(out(I,j), 'vars');
    varnames=get(out(I,j), 'varnames');
    vars(2)=j;
    varnames{2}='channel';
    vars(3)= chi.starttime; % +chi.starttime/24/3600/1000;??
    varnames{3}='serial time(d)';
    out(I,j)=set(out(I,j), 'vars', vars);
    out(I,j)=set(out(I,j), 'varnames', varnames);

    rhd_data.curvedata(I,j)=compress(out(I,j), 2);
end
% if ishandle(ha), waitbar(j /length(amplifier_channels), ha), else ha = waitbar(0,'Reading file...'); end
% 
% if ishandle(ha), close(ha), end

% DIGITAL INPUT:
try
    dt= t_amplifier(2)- t_amplifier(1); % feltetelezzuk, h ekvidisztans és ms-ban van
%     if sum(abs(board_dig_in_data - board_dig_in_raw))>0
%         warning('board_dig_in_data nem egyezik meg board_dig_in_raw-val')
%     end
%     %measurement date
%     rhd_data.FileStartDate = ['20',filename(end-16:end-11)];  % mert 20xx-es kezdetûevekben jarunk..
%     rhd_data.FileStartTime = 1000*((str2double(filename(end-9:end-8))*60 + str2double(filename(end-7:end-6))) *60 + str2double(filename(end-5:end-4)));
...
%         M=str2double(date(5:6));
%         D=str2double(date(7:8));
%         board_dig_in_data.starttime =datenum(Y,M,D)+time/24/3600/1000;
%     end

    %endtime=board_dig_in_data.starttime+t_amplifier(end);
    
    name=[filename,'__', board_dig_in_channels.custom_channel_name];
%     if j==1, I=I+1; end
    temp=[];
    temp.name=name;
    temp.csoport=board_dig_in_channels.port_number;
    temp.xunit='ms';
    temp.xname=xname;
    temp.yunit=[char(0181),'V']; % mu kodja
    temp.yname = board_dig_in_channels.port_name;
    temp.axis = 2;
    ...
    temp=gorobj('eqsamp', [tfactor*t_amplifier(1), tfactor*(dt) ], 'double', board_dig_in_data, temp);
    out(I,6)=temp;
    
    vars=get(out(I,end), 'vars');
    varnames=get(out(I,end), 'varnames');
    vars(2)=board_dig_in_channels.port_number;
    varnames{2}='channel';
    vars(3)= chi.starttime; %uaz, mint a chi-nek
    varnames{3}='serial time(d)';
    out(I,end)=set(out(I,end), 'vars', vars);
    out(I,end)=set(out(I,end), 'varnames', varnames);  
    rhd_data.curvedata(I,end+1)=compress(out(I,end), 2);
catch
    warning('Digital input channel could not be appended.')
end
